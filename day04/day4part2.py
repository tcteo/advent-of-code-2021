...
def main():
  ...
  for num in numbers:
    ...
    wb = winning_boards(hits)
    new_winning_boards = set(wb) - set(prev_wb)
    print(f'               {np.sum(hits)} hits total')
    print(f'               {len(wb)} winning boards: {wb}')
    print(f'               boards won this round: {new_winning_boards}')
    prev_wb = wb
    if len(new_winning_boards)>0:
      winning_board_unmarked_sum = board_unmarked_sum(boards, hits, list(new_winning_boards)[0])
